...
def simplifier(tree):
    if isinstance(tree, tuple) and not isinstance(tree, Token):
        ...
        if tree[0] in ('assignment', 'comparision', 'equality', 'logic_or', 'logic_and', 'term', 'factor', 'unary') and len(tree) == 2:
             return simplifier(tree[1])
        else:
